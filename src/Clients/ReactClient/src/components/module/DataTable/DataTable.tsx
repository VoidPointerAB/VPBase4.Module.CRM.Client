...
export function rowIdsGetter(idName: string, selector?: string): string[] {
    const tableSelector = selector ? selector : '.dataTables_wrapper table';
    const rowIds = [];
    const dataTable = $(tableSelector).DataTable();
    const rowData = dataTable.rows({ filter: 'applied' }).data();

    for (let i = 0; i < rowData.length; i++) {
        rowIds.push(rowData[i][idName]);
    }
